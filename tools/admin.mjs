...

{
  ...
  ok(u.def.hp === hp * 2 && u.def.damage === dmg + 5 && waveCount('m1', 0, 0) === 99,
    'three edits take', `${u.def.hp} / ${u.def.damage} / ${waveCount('m1', 0, 0)}`);

  reset();
  ok(!touched(), 'reset empties the store');
  ok(u.def.hp === hp && u.def.damage === dmg,
    'and puts the DEFS back too', `${u.def.hp} / ${u.def.damage}`);
  ok(waveCount('m1', 0, 0) === shipped('m1|0|0'), 'and the wave counts with them');
}
...

{
  ...
  setWaveCount('m1', 0, 0, 0);
  ok(waveCount('m1', 0, 0) === 1, 'a group never drops below one enemy');
  setWaveCount('m1', 0, 0, 5000);
  ok(waveCount('m1', 0, 0) === 99, 'and never above ninety-nine');
  reset();
